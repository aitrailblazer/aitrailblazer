...
// LoadConceptsFromJSON unmarshals the provided JSON string (which should be a JSON array
// of concept objects) into a slice and then builds a map keyed by the concept's name.
func LoadConceptsFromJSON(jsonStr string) (map[string]Concept, error) {
	var conceptsSlice []Concept
	if err := json.Unmarshal([]byte(jsonStr), &conceptsSlice); err != nil {
		return nil, err
	}

	conceptMap := make(map[string]Concept)
	for _, concept := range conceptsSlice {
...
// is updated to "[G]" plus the GAAP title. Otherwise, the cleaned key is formatted as a title.
